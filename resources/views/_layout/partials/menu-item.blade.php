@php
    // Завантажуємо верхній рівень меню, якщо ще не передано $menu
    if (!isset($menu)) {
        $menuItems = app(\App\Http\Controllers\MenuController::class)->getMenu();
    } else {
        $menuItems = [$menu]; // Якщо передано конкретний елемент, рендеримо його
    }
@endphp
